...
<main class="screen">
	<section class="totem-card">
		<div class="totem-inner">
			<header class={`totem-header ${mode === 'child' ? 'is-child' : ''}`}>
			...
			{#if mode === 'child'}
				<nav class="bottom-actions" aria-label="Navigazione pagina">
					...
					<a href="/chat-assistenza" class="bottom-assist" aria-label="Apri chat assistenza">
						<img src="/img/DONAtello_AI.png" alt="Chat assistenza" class="bottom-assist-icon" />
					</a>
				</nav>
			{/if}
		</div>
